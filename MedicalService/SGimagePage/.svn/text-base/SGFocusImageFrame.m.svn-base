...
- (id)initWithFrame:(CGRect)frame delegate:(id<SGFocusImageFrameDelegate>)delegate focusItems:(NSArray *)imageItems
{
    self = [super initWithFrame:frame];
    if (self) {
        //        NSMutableArray *imageItems = [NSMutableArray array];
...
- (void)setupViews
{
    ...
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(self.bounds.origin.x, self.bounds.origin.y, self.bounds.size.width, self.bounds.size.height-10)];
    
    CGSize size = CGSizeMake(100, 44);
    _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(self.bounds.size.width *.5 - size.width *.5, self.bounds.size.height - size.height+8, size.width, size.height)];
    ...
    */
    ...
    for (int i = 0; i < imageItems.count; i++) {
        ...
        [imageView setFrame:CGRectMake(i * _scrollView.frame.size.width, 0, _scrollView.frame.size.width, _scrollView.frame.size.height-40)];
...
//        imageView.image = item.image;
        //添加标题栏
        UILabel * lbltitle = [[UILabel alloc] initWithFrame:CGRectMake(i *_scrollView.frame.size.width+3, _scrollView.frame.size.height-37.0, _scrollView.frame.size.width, 22.0)];
...
-(void)clickPageImage:(UIButton *)sender{
    ...
    if ([_delegate respondsToSelector:@selector(showHospitalDetail:)]) {
        [self.delegate showHospitalDetail:sender.tag];
    }
}
